...
@Component({
  selector: 'app-add-new-revision-dialog',
  templateUrl: './add-new-revision-dialog.component.html',
  styleUrls: ['./add-new-revision-dialog.component.scss']
})
export class AddNewRevisionDialogComponent implements OnInit {
  ...
  async uploadDoc(file, uploadedDocument) {
    ...
    newDoc.size = this.transformFileSize(file.size, 1)

    this.apiService.post(ApiURL.documentss, newDoc).subscribe(res => {
