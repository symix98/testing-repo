...
@Component({
  selector: 'app-documents-sub-type',
  templateUrl: './documents-sub-type.component.html',
  styleUrls: ['./documents-sub-type.component.scss']
})

export class DocumentsSubTypeComponent implements OnInit {
  ...
  async initTableParam() {
    ...
    param.api = ApiURL.documents_sub_types;
  ...
  onRowEdit = (event): Promise<void> => {
    return new Promise((resolve, reject) => {
      this.apiService.put(ApiURL.documents_sub_types + "/" + event.object.id, event.object).subscribe(res => {
        this.utilities.notifySuccess("Document Sub Type Updated Successfully!");
      })
      resolve();
    });
  }
  ...
  onRowDelete = (event): Promise<void> => {
    return new Promise((resolve, reject) => this.utilities.confirmDialog("Are you sure you want to delete this Document Sub Type?").then((confirm) => {
      if (confirm) {
        this.apiService.delete(ApiURL.documents_sub_types + "/" + event.selectedObjects[0].id).subscribe(res => {
          this.utilities.notifySuccess("Document Sub Type Deleted Successfully!");
