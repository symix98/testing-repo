...
export class RecipientComponent implements OnInit {
  ...
  async initTableParam() {
    ...
    param.api = ApiURL.recipientIdRefs;
  ...
  initTableColumns(): TableColumn[] {
    let tableColumns: TableColumn[] =
      [
        ...
        { title: 'Code', field: 'code', fieldType: FieldType.string, width: '20%', filter: { filterType: FilterType.string, filterName: 'code' } },
  ...
  onRowEdit = (event): Promise<void> => {
    return new Promise((resolve, reject) => {
      this.apiService.put(ApiURL.recipientIdRefs + "/" + event.object.id, event.object).subscribe(res => {
        this.utilities.notifySuccess("Recipient Updated Successfully!");
      })
      resolve();
    });
  }
  ...
  onRowDelete = (event): Promise<void> => {
    return new Promise((resolve, reject) => this.utilities.confirmDialog("Are you sure you want to delete this Recipient?").then((confirm) => {
      if (confirm) {
        this.apiService.delete(ApiURL.recipientIdRefs + "/" + event.selectedObjects[0].id).subscribe(res => {
          this.utilities.notifySuccess("Recipient Deleted Successfully!");
