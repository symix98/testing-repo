...
@Component({
  selector: 'app-files-type',
  templateUrl: './files-type.component.html',
  styleUrls: ['./files-type.component.scss']
})

export class FilesTypeComponent implements OnInit {
  ...
  onRowEdit = (event): Promise<void> => {
    return new Promise((resolve, reject) => {
      this.apiService.put(ApiURL.files_types + "/" + event.object.id, event.object).subscribe(res => {
        this.utilities.notifySuccess("File Type Updated Successfully!");
      })
      resolve();
    });
  }
  ...
  onRowDelete = (event): Promise<void> => {
    return new Promise((resolve, reject) => this.utilities.confirmDialog("Are you sure you want to delete this File Type?").then((confirm) => {
      if (confirm) {
        this.apiService.delete(ApiURL.files_types + "/" + event.selectedObjects[0].id).subscribe(res => {
          this.utilities.notifySuccess("File Type Deleted Successfully!");
